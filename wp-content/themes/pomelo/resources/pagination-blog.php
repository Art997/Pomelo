...
function boot_pagination( $args = array() ) {
	...
	$pagination .= "\n" . '<nav aria-label="' . $page_title . '"><ul class="list-unstyled mb-0 pagination justify-content-center">' . "\n";

	// Previous Post Link

	if ( $prev_next && get_previous_posts_link() ) {
		$pagination .= sprintf( '<li class="page-item prev"><a class="page-link" href="%s"> <<
	  </a></li>' . "\n", get_previous_posts_page_link() );		
	} else {
        $pagination .= '<li class="page-item next disabled"><a class="page-link" href="%s"> <
	...
	if ( ! in_array( 1, $links ) ) {
		...
		$pagination .= sprintf( '<li class="page-item%s"><a class="page-link" href="%s">0%s</a></li>', $class, esc_url( get_pagenum_link( 1 ) ), '1' );
		$pagination .= "\n";
		if ( ! in_array( 2, $links ) ) {
	...
	foreach ( (array) $links as $link ) {
		...
		$pagination .= sprintf( '<li class="page-item%s"><a class="page-link" href="%s">0%s</a></li>', $class, esc_url( get_pagenum_link( $link ) ), $link );
		$pagination .= "\n";
	}
	...
	if ( ! in_array( $max, $links ) ) {
		...
		$pagination .= sprintf( '<li%s><a class="page-link" href="%s">0%s</a></li>' . "\n", $class, esc_url( get_pagenum_link( $max ) ), $max );
		$pagination .= "\n";
	}

	// Next Post Link

	if ( $prev_next && $paged < $max ) {
		$pagination .= sprintf( '<li class="page-item next"><a class="page-link" href="%s"> >>
